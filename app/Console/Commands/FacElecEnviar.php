...
class FacElecEnviar extends Command
{
    ...
    {
        $firmados = Factura::with('contribuyente')->where('estado', 'F')->whereNull('identificador')->get();
        ...
        foreach($firmados as $firmado) {
            try {
                ...
                $url = $firmado['contribuyente']['tipo_ambiente'] == 1 ? env('SRI_PRUEBAS') : env('SRI_PRODUCCION');
                ...
                if (($result->RespuestaRecepcionComprobante->estado) == "RECIBIDA") {
                    ...
                    Factura::where(['clave_acceso' => $firmado['clave_acceso']])->update($datosAct);
                } else {
